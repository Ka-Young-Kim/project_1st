...
test("hides portfolio composition CTA when no account is selected", async ({
  ...
  const sidebarLinks = page.locator("nav").first().getByRole("link");
  await expect(sidebarLinks.nth(0)).toHaveAccessibleName(
    /투자일지 매매 기록과 회고/,
  );
  await expect(sidebarLinks.nth(1)).toHaveAccessibleName(
    /계좌 추가 직접 등록과 불러오기/,
  );

  await goToNavLink(page, /계좌 추가 직접 등록과 불러오기/);
  await expect(page).toHaveURL(/\/accounts\?portfolio=/);
  await expect(page.getByRole("heading", { name: "계좌 추가" })).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "계좌를 선택하세요" }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: "포트폴리오 구성 열기" }),
  ).toHaveCount(0);
});
...
test("manages accounts and keeps first-journal setup inside the journal page", async ({
  ...
  await createJournalForm
    .getByLabel("계좌")
    .selectOption({ label: updatedAccountName });
  await createJournalForm
    .getByLabel("투자 항목")
  ...
  await page.getByLabel("포트폴리오 홈").click();
